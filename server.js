// ============================================
// 🖤 HODEKAI BOT v5.0 - COMPLETE FIXED VERSION
// AUTO-WHITELIST + FRESH SESSION
// ============================================
...
// ─── DATA FOLDERS (FRESH SESSION) ──────────
const DATA_DIR = path.join(__dirname, 'data');
const DATA_FILE = path.join(DATA_DIR, 'conclave_data.json');
const AUTH_DIR = path.join(__dirname, 'auth_info_baileys_v2'); // ✅ FRESH SESSION
...
// ✅ FIXED: ALL groups are now whitelisted automatically!
function isGroupWhitelisted(g) {
    return true; // 🎉 Bot works in EVERY group!
}
...
function handleMessage(msg, userNumber, isSticker, groupId) {
    ...
    // ✅ No more group whitelist check - works everywhere!
    ...
    if (cmd === 'govjob') {
        user.job = { title: "Government Employee", salary: 75, shifts: 0, lastWork: 0 };
    ...
    if (cmd === 'viewall') {
        ...
        for (let num in USERS) {
            output += "📱 " + num + ": " + USERS[num].role + " - " + USERS[num].xenoShards + " XS\n";
        }
        return output + "\n" + getMixedResponse();
    }
